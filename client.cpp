...
while ((opt = getopt(argc, argv, "p:t:e:f:m:c")) != -1) {
    ...
    if (p != -1 && t == -1.0) {
    t = 0.0;  // default time if not specified
    }
...
if (filename != "") {
    ...
    memcpy(msg, &fmsg, sizeof(filemsg));
    strcpy(msg + sizeof(filemsg), filename.c_str());
    active_chan->cwrite(msg, msg_len);
    int64_t file_size;
    active_chan->cread(&file_size, sizeof(int64_t));
    ...
    // Step 3: Transfer file in chunks
    int64_t offset = 0;
    char* response = new char[buffercapacity];
    while (offset < file_size) {
        ...
        active_chan->cwrite(msg, msg_len);
        int bytes_read = active_chan->cread(response, chunk_size);
        if (bytes_read != chunk_size) {
            break;
        }
        outfile.write(response, bytes_read);
        offset += chunk_size;
    }
